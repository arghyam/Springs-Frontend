...
class ReviewerDataRepository @Inject constructor() {


    fun reviewerDataApiRequest(context: Context, requestModel: RequestModel, responseListener: ResponseListener<ResponseModel>) {
        val reviewerCall = RestClient.getWebServiceData()?.reviewerData(requestModel)
        reviewerCall?.enqueue(object : Callback<ResponseModel> {
