...
class NotificationCountRepository @Inject constructor() {


    fun notificationCountApiRequest(context: Context, userId: String, requestModel: RequestModel, responseListener: ResponseListener<ResponseModel>) {
        val notificationCountCall = RestClient.getWebServiceData()?.notificationCount(userId,requestModel)
        notificationCountCall?.enqueue(object : Callback<ResponseModel> {
